...
Before broadcasting events, you will also need to configure and run a [queue listener](/laravel_tw/docs/5.3/queues). All event broadcasting is done via queued jobs so that the response time of your application is not seriously affected.
...
Then, you only need to [fire the event](/laravel_tw/docs/5.3/events) as you normally would. Once the event has been fired, a [queued job](/laravel_tw/docs/5.3/queues) will automatically broadcast the event over your specified broadcast driver.
