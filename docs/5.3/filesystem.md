...
In order to set up the custom filesystem you will need to create a [service provider](/laravel_tw/docs/5.3/providers) such as `DropboxServiceProvider`. In the provider's `boot` method, you may use the `Storage` facade's `extend` method to define the custom driver:
