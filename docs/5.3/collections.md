...
> {tip} The results of [Eloquent](/laravel_tw/docs/5.3/eloquent) queries are always returned as `Collection` instances.
...
This method is especially useful in [views](/laravel_tw/docs/5.3/views) when working with a grid system such as [Bootstrap](https://getbootstrap.com/css/#grid). Imagine you have a collection of [Eloquent](/laravel_tw/docs/5.3/eloquent) models you want to display in a grid:
...
The `toArray` method converts the collection into a plain PHP `array`. If the collection's values are [Eloquent](/laravel_tw/docs/5.3/eloquent) models, the models will also be converted to arrays:
