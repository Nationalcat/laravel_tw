...
The `VerifyCsrfToken` [middleware](/laravel_tw/docs/master/middleware), which is included in the `web` middleware group, will automatically verify that the token in the request input matches the token stored in the session.
