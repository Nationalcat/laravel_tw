...
Any HTML forms pointing to `POST`, `PUT`, or `DELETE` routes that are defined in the `web` routes file should include a CSRF token field. Otherwise, the request will be rejected. You can read more about CSRF protection in the [CSRF documentation](/laravel_tw/docs/master/csrf):
...
Refer to the API documentation for both the [underlying class of the Route facade](http://laravel.com/api/{% raw %} {{version}} {% endraw %}/Illuminate/Routing/Router.html) and [Route instance](http://laravel.com/api/{% raw %} {{version}} {% endraw %}/Illuminate/Routing/Route.html) to review all accessible methods.
