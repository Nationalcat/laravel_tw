...
To convert a model and its loaded [relationships](/laravel_tw/docs/master/eloquent-relationships) to an array, you should use the `toArray` method. This method is recursive, so all attributes and all relations (including the relations of relations) will be converted to arrays:

    $user = App\User::with('roles')->first();

    return $user->toArray();

You may also convert entire [collections](/laravel_tw/docs/master/eloquent-collections) of models to arrays:
...
Occasionally, when casting models to an array or JSON, you may wish to add attributes that do not have a corresponding column in your database. To do so, first define an [accessor](/laravel_tw/docs/master/eloquent-mutators) for the value:
