...
Before getting started, be sure to configure a database connection in `config/database.php`. For more information on configuring your database, check out [the documentation](/laravel_tw/docs/5.4/database#configuration).
...
The easiest way to create a model instance is using the `make:model` [Artisan command](/laravel_tw/docs/5.4/artisan):

    php artisan make:model User

If you would like to generate a [database migration](/laravel_tw/docs/5.4/migrations) when you generate the model, you may use the `--migration` or `-m` option:
...
Once you have created a model and [its associated database table](/laravel_tw/docs/5.4/migrations#writing-migrations), you are ready to start retrieving data from your database. Think of each Eloquent model as a powerful [query builder](/laravel_tw/docs/5.4/queries) allowing you to fluently query the database table associated with the model. For example:
...
The Eloquent `all` method will return all of the results in the model's table. Since each Eloquent model serves as a [query builder](/laravel_tw/docs/5.4/queries), you may also add constraints to queries, and then use the `get` method to retrieve the results:
...
> {tip} Since Eloquent models are query builders, you should review all of the methods available on the [query builder](/laravel_tw/docs/5.4/queries). You may use any of these methods in your Eloquent queries.

<a name="collections"></a>
### Collections

For Eloquent methods like `all` and `get` which retrieve multiple results, an instance of `Illuminate\Database\Eloquent\Collection` will be returned. The `Collection` class provides [a variety of helpful methods](/laravel_tw/docs/5.4/eloquent-collections#available-methods) for working with your Eloquent results:
...
You may also use the `count`, `sum`, `max`, and other [aggregate methods](/laravel_tw/docs/5.4/queries#aggregates) provided by the [query builder](/laravel_tw/docs/5.4/queries). These methods return the appropriate scalar value instead of a full model instance:
...
Of course, you should add the `deleted_at` column to your database table. The Laravel [schema builder](/laravel_tw/docs/5.4/migrations) contains a helper method to create this column:
...
The `withTrashed` method may also be used on a [relationship](/laravel_tw/docs/5.4/eloquent-relationships) query:
...
Like the `withTrashed` method, the `restore` method may also be used on [relationships](/laravel_tw/docs/5.4/eloquent-relationships):
...
To get started, define an `$events` property on your Eloquent model that maps various points of the Eloquent model's lifecycle to your own [event classes](/laravel_tw/docs/5.4/events):
