...
The `VerifyCsrfToken` [middleware](/laravel_tw/docs/5.4/middleware), which is included in the `web` middleware group, will automatically verify that the token in the request input matches the token stored in the session.
