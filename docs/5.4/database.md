...
Laravel makes interacting with databases extremely simple across a variety of database backends using either raw SQL, the [fluent query builder](/laravel_tw/docs/5.4/queries), and the [Eloquent ORM](/laravel_tw/docs/5.4/eloquent). Currently, Laravel supports four databases:
...
By default, Laravel's sample [environment configuration](/laravel_tw/docs/5.4/configuration#environment-configuration) is ready to use with [Laravel Homestead](/laravel_tw/docs/5.4/homestead), which is a convenient virtual machine for doing Laravel development on your local machine. Of course, you are free to modify this configuration as needed for your local database.
...
If you would like to receive each SQL query executed by your application, you may use the `listen` method. This method is useful for logging queries or debugging. You may register your query listener in a [service provider](/laravel_tw/docs/5.4/providers):
...
> {tip} Using the `DB` facade's transaction methods also controls transactions for the [query builder](/laravel_tw/docs/5.4/queries) and [Eloquent ORM](/laravel_tw/docs/5.4/eloquent).
