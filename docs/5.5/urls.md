...
Each of these methods may also be accessed via the `URL` [facade](/laravel_tw/docs/5.5/facades):
...
You will often be generating URLs using the primary key of [Eloquent models](/laravel_tw/docs/5.5/eloquent). For this reason, you may pass Eloquent models as parameter values. The `route` helper will automatically extract the model's primary key:
...
It is cumbersome to always pass the `locale` every time you call the `route` helper. So, you may use the `URL::defaults` method to define a default value for this parameter that will always be applied during the current request. You may wish to call this method from a [route middleware](/laravel_tw/docs/5.5/middleware#assigning-middleware-to-routes) so that you have access to the current request:
