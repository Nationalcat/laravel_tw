...
預設 [Eloquent](/laravel_tw/docs/5.2/eloquent) 模型的集合總是以 `Collection` 實例回傳；然而，你可以任意在你應用程式任何適當的場合使用 `Collection` 類別。
...
這個方法在有用到網格系統如 [Bootstrap](http://getbootstrap.com/css/#grid) 的[視圖](/laravel_tw/docs/5.2/views)內特別有用。想像你有一個 [Eloquent](/laravel_tw/docs/5.2/eloquent) 模型的集合要顯示在一個網格內：
...
`toArray` 方法將集合轉換成純 PHP `陣列`。假如集合的數值是 [Eloquent](/laravel_tw/docs/5.2/eloquent) 模型，也會被轉換成陣列：
