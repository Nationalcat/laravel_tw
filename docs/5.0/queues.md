...
> **注意:** 在這個範例當中，我們直接使用 `Queue` Facade，然而，常見的作法是藉由 [Command Bus](/laravel_tw/docs/5.0/bus) 是分派隊列指令。我們將會在整篇文章中繼續使用 `Queue` Facade，不過，也要熟悉使用 command bus，因為它能夠同時分派你的網站應用程式中隊列與同步的指令。
