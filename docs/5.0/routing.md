...
當然也可以在 Blade [模板引擎](/laravel_tw/docs/5.0/templates)使用：

	<input type="hidden" name="_token" value="{% raw %} {{ csrf_token() }} {% endraw %}">

	您不需要手動驗證 POST、PUT 或 DELETE 請求的 CSRF token。`VerifyCsrfToken` [HTTP  中介層](/laravel_tw/docs/5.0/middleware)將自動驗證請求與 Session 中的 token 是否相符合。
...
有關如何處理 404 例外狀況和自定回應的更多資訊，可以參考[錯誤](/laravel_tw/docs/5.0/errors#http-exceptions)章節內的文件。
