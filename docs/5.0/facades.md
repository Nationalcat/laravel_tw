...
註冊這個綁定的好方式是建立新的 [服務提供者](/laravel_tw/docs/5.0/container#service-providers) 命名為 `PaymentServiceProvider`，並把這個綁定加到 `register` 方法。再來你可以設定 Laravel 從 `config/app.php` 設定檔載入你的服務提供者。
