...
一開始，在你的本地環境中[安裝新的 Laravel 5 應用程式](/laravel_tw/docs/5.0/installation)在一個全新的目錄下，我們將詳細探討整合各部分的過程。
...
更多關於環境設定值，請見[完整文件](/laravel_tw/docs/5.0/configuration#environment-configuration)。
...
預設情況下，所有路由都會使用 [CSRF 保護](/laravel_tw/docs/5.0/routing#csrf-protection)。若想關閉它們，或是只在特定路由開啟，請移除 `App\Http\Kernel` 中 `middleware` 陣列內的這一行：
...
現在，您可於路由內使用 `['middleware' => 'csrf']` 即可個別添加中介層到路由/控制器。了解更多關於中介層，請見[完整文件](/laravel_tw/docs/5.0/middleware)。
...
Eloquent 不再提供 `remember` 方法來快取查詢結果。現在你需要手動使用 `Cache::remember` 方法快取查詢結果。了解更多關於快取，請見[完整文件](/laravel_tw/docs/5.0/cache)。
...
[Laravel Cashier](/laravel_tw/docs/5.0/billing) 的 trait 和介面名稱已作修改。trait 請改用 `Laravel\Cashier\Billable` 取代 `BillableTrait`。介面請改用 `Laravel\Cashier\Contracts\Billable` 取代 `Laravel\Cashier\BillableInterface`。不需要修改任何方法。
...
若您在 `start/global.php` 有綁定任何 [IoC](/laravel_tw/docs/5.0/container)，請將它們移到 `app/Providers/AppServiceProvider.php` 內的 `register` 方法中，你可能需要引入 `App` facade。
...
基於安全考量，Laravel 5.0 會轉譯所有輸出，不論您使用 `{% raw %} {{ }} {% endraw %}` 或 `{% raw %} {{{ }} {% endraw %}}` 標籤。您可以使用新的 `{!! !!}` 標籤來取消輸出轉譯。請務必**確定**輸出內容是安全的才使用 `{!! !!}` 標籤。
...
你不該輕易的使用上述設定，這將使您的應用程式更加容易暴露於 XSS 攻擊，而且註解 `{% raw %} {{--` 將無法作用。
...
如果您使用表單或 HTML 輔助函數，您將會看到以下錯誤 `class 'Form' not found` 或 `class 'Html' not found`。 The Form and HTML helpers have been deprecated in Laravel 5.0; however, there are community-driven replacements such as those maintained by the [Laravel Collective](http://laravelcollective.com/{% raw %} {{version}} {% endraw %}/5.0/html).
