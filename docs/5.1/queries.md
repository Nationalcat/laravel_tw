...
就像[原始查詢](/laravel_tw/docs/5.1/database)，`get` 方法會回傳一個結果`陣列`，其中每一個結果都是 PHP `StdClass` 物件的實例。你可以將欄位作為物件的性質，來存取每個欄位的值：
...
若你需要操作數千筆資料庫記錄，可考慮使用 `chunk` 方法。這個方法一次取出一小「塊」結果，並將每個區塊餵給一個`閉包`處理。這個方法對編寫要處理數千筆記錄的 [Artisan 指令](/laravel_tw/docs/5.1/artisan)非常有用。例如，讓我們將整個 `user` 資料表切塊，一次處理 100 筆記錄：
