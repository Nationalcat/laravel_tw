...
預設的 Laravel 在你的 `app` 資料夾中含有 `App\User` [Eloquent 模型](/laravel_tw/docs/5.1/eloquent)。這個模型使用預設的 Eloquent 認證來驅動。如果你的應用程式沒有使用 Eloquent，你可以使用 Laravel 查詢生成器的 `database` 認證驅動。
...
預設中，沒有[路由](/laravel_tw/docs/5.1/routing)指向這些認證控制器，你需要自己新增到 `app/Http/routes.php` 中。
...
雖然這些認證控制器被包含在框架中，你仍需提供[視圖](/laravel_tw/docs/5.1/views)給控制器來算圖（render），而視圖需要被放置在 `resources/views/auth` 資料夾中，你可以任意的客製化此視圖。登入視圖應該被放在 `resources/views/auth/login.blade.php` 而註冊視圖則放在 `resources/views/auth/register.blade.php`。
...
`AuthController` 的 `create` 方法負責使用 [Eloquent ORM](/laravel_tw/docs/5.1/eloquent) 創造新的 `App\User` 紀錄到你的資料庫。你可以根據需求任意修改這個方法。
...
不過，在允許該使用者存取特定的路由或控制器之前，你可以使用中介層來確認這個使用者是否認證過。想得到更多資訊，請閱讀[保護路由](/laravel_tw/docs/5.1/authentication#protecting-routes)的文件。

<a name="protecting-routes"></a>
### 保護路由

[路由中介層](/laravel_tw/docs/5.1/middleware)使用於限定認證過的使用者存取指定的路由，Laravel 提供了 `auth` 中介層來達到這個目的，而這個中介層被定義在 `app\Http\Middleware\Authenticate.php` 中，你只需要將它應用到路由定義中：
...
當然，如果你正在使用[控制器類別](/laravel_tw/docs/5.1/controllers)，你可以在建構子中呼叫 `middleware` 方法，而不是在路由中直接定義它：
...
我們將透過 `Auth` [facade](/laravel_tw/docs/5.1/facades) 存取 Laravel 的認證服務，所以我們需要確認是否在類別的最上面引入 `Auth` facade，接下來讓我們看一下 `attempt` 方法：
...
如果你需要使用存在的使用者實例來登入，你需要呼叫 `login` 方法，並傳入使用實例，這個物件必須是由 `Illuminate\Contracts\Auth\Authenticatable` [contract](/laravel_tw/docs/5.1/contracts) 所實現。當然，`App/User` 模型已經實現了這個介面：
...
[HTTP 基礎認證](http://en.wikipedia.org/wiki/Basic_access_authentication)提供一個快速的方法來認證使用者，不需要任何「登入」頁面。開始之前，先增加 `auth.basic` [中介層](/laravel_tw/docs/5.1/middleware)到你的路由，`auth.basic` 中介層已經被包含在 Laravel 框架中，所以你不需要定義它：
...
你可以使用 HTTP 基礎認證而不用在 session 中設定使用者認證用的 cookie，這個功能對 API 認證來說非常有用。為了達到這個目的，[定義一個中介層](/laravel_tw/docs/5.1/middleware)並這個中介層會呼叫 `onceBasic` 方法。如果沒有任何回應從 `onceBasic` 方法返回的話，這個請求會直接傳進應用程式中：
...
接著，[註冊這個路由中介層](/laravel_tw/docs/5.1/middleware#registering-middleware)，然後將它增加在一個路由上：
...
當使用者送出重置密碼的請求，他們會收到一封有連結到 `PasswordController` 的 `getReset` 方法（通常是路由到 `/password/reset`）的電子郵件。你將需要為電子郵件創造一個 `resources/views/emails/password.blade.php` 視圖。這個視圖會接收一個帶有密碼重置標記的 `$token` 變數，這個變數含有了密碼重置標記來匹配使用者的密碼重置請求，以下是範例來讓你開始：
    ...
    點擊此處重置你的密碼：{% raw %} {{ url('password/reset/'.$token) }} {% endraw %}
...
接下來，你已經準備好開始認證使用者了！你將需要兩個路由: 一個用來重導使用者到 OAuth 提供者，另一個在認證後接收提供者的回呼。我們將會藉由 `Socialite` [facade](/laravel_tw/docs/5.1/facades) 存取 Socialite：
...
如果你不是使用傳統的關聯式資料庫來儲存使用者，你將需要擴充 Laravel 來新增你自己的認證驅動。我們將使用 `Auth` facade 的 `extend` 方法來定義客製化驅動。你應該將 `extend` 放置在[服務提供者](/laravel_tw/docs/5.1/providers)中：
...
Laravel 提供了在認證過程中的各種[事件](/laravel_tw/docs/5.1/events)。你可以在 `EventServiceProvider` 為這些事件連接監聽器：
