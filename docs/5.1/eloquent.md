...
在開始之前，請確認有設定你的資料庫連結在 `config/database.php` 檔案內。更多資料庫的設定資訊，請查看[資料庫設定](/laravel_tw/docs/5.1/database#configuration)。
...
建立模型實例的最簡單的方法是使用 `make:model` [Artisan 指令](/laravel_tw/docs/5.1/artisan)：

    php artisan make:model User

假設當你生成一個模型時，想要產生一個[資料庫遷移](/laravel_tw/docs/5.1/schema#database-migrations)，可以使用 `--migration` 或 `-m` 選項：
...
一旦你建立了一個模型並且將模型[關連到資料表](/laravel_tw/docs/5.1/schema)，你就可以從資料庫中取得資料。把每個 Eloquent 模型想像成強大的[查詢建構器](/laravel_tw/docs/5.1/queries)，讓你可以流暢地查詢與模型關聯的資料表。例如：
...
Eloquent 的 `all` 方法會回傳在模型資料表中所有的結果。由於每個 Eloquent 模型可以當作一個[查詢建構器](/laravel_tw/docs/5.1/queries)，所以你可以在查詢中增加規則，然後透過 `get` 方法來取得結果：
...
> **注意：**由於 Eloquent 模型是查詢建構器，應該檢閱所有[查詢建構器](/laravel_tw/docs/5.1/queries)可用的方法。你可以在你的 Eloquent 查詢中使用這其中的任何方法。

#### 集合

像是 `all` 以及 `get` 之類可以取回多筆結果的 Eloquent 方法，將會回傳一個 `Illuminate\Database\Eloquent\Collection` 實例。`Collection` 類別提供[多樣的輔助方法](/laravel_tw/docs/5.1/eloquent-collections#available-methods) 用來處理你的 Eloquent 結果。當然，你也可以簡單地像陣列一樣遍歷你的集合：
...
當然，你也可以使用 `count`、`sum`、`max`，和其他[查詢建構器](/laravel_tw/docs/5.1/queries)提供的[聚合函式](/laravel_tw/docs/5.1/queries#aggregates)。這些方法會回傳適當的純量值，而不是一個完整的模型實例：
...
當然，你應該添加 `deleted_at` 欄位到你的資料表。Laravel [結構產生器](/laravel_tw/docs/5.1/migrations)包含了一個輔助的方法用來建立這個欄位：
...
`withTrashed` 方法也可以被用在[關聯](/laravel_tw/docs/5.1/eloquent-relationships)查詢：
...
與 `withTrashed` 方法類似，`restore` 方法也可以被用在[關聯](/laravel_tw/docs/5.1/eloquent-relationships):
...
例如，讓我們來在[服務提供者](/laravel_tw/docs/5.1/providers)中定義一個 Eloquent 事件監聽器。在我們的事件監聽器中，我們會在給定的模型上呼叫 `isValid` 方法，並在模型無效的時候回傳 `false`。從 Eloquent 事件監聽器回傳 `false` 會取消 `save` 和 `update` 的操作：
