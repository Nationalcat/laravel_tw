...
當在你的中介層呼叫 `terminate` 方法時，Laravel 會從[服務容器](/laravel_tw/docs/5.1/container)解析一個全新的中介層實例。如果你希望在 `handle` 及 `terminate` 方法被呼叫使用一致的中介層實例，只需在容器中使用容器的 `singleton` 方法註冊中介層。
