...
所有的服務提供者都繼承了 `Illuminate\Support\ServiceProvider` 類別。這個抽象類別要求你在你的提供者上定義至少一個方法：`register`。在 `register` 方法中，你應該**只將事物綁定至[服務容器](/laravel_tw/docs/5.1/container)之中**。你永遠不該試圖在 `register` 方法中註冊任何事件監聽器、路由或任何其他功能。
...
如同之前提到的，在 `register` 方法中，你應該只將事物綁定至[服務容器](/laravel_tw/docs/5.1/container)中。你永遠不該試圖在 `register` 方法中註冊任何事件監聽器、路由或任何其他功能。否則的話，你可能會意外地使用到由尚未載入的服務提供者所提供的服務。
...
此服務提供者只定義了一個 `register` 方法，並在服務容器中使用此方法定義了一份 `Riak\Contracts\Connection` 的實作。若你不瞭解服務容器是如何運作的，可查閱[其文件](/laravel_tw/docs/5.1/container)。
...
我們可以為我們 `boot` 方法中的依賴作型別提式。[服務容器](/laravel_tw/docs/5.1/container)會自動注入你所需要的任何依賴：
...
若你的提供者**僅**於[服務容器](/laravel_tw/docs/5.1/container)中註冊綁定，你可以選擇延緩其註冊，直到真正需要其中已註冊的綁定。延緩像這樣的提供者載入可增進你的應用程式的效能，因為這樣就毋須每個請求都從檔案系統將其載入。
