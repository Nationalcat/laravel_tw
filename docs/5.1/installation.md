...
Laravel 框架有一些系統上的需求。當然，[Laravel Homestead](/laravel_tw/docs/5.1/homestead) 虛擬機器都能滿足這些需求：
...
安裝 Laravel 之後，你必須設定一些權限。`storage` 和 `bootstrap/cache` 目錄必須讓伺服器有寫入權限。如果你使用 [Homestead](/laravel_tw/docs/5.1/homestead) 虛擬機器，那麼這些權限應該已經被設定完成。
...
- [快取](/laravel_tw/docs/5.1/cache#configuration)
- [資料庫](/laravel_tw/docs/5.1/database#configuration)
- [Session](/laravel_tw/docs/5.1/session#configuration)

一旦 Laravel 安裝完成，你應該同時[設定本機環境](/laravel_tw/docs/5.1/installation#environment-configuration)。
...
當然，如果你使用 [Homestead](/laravel_tw/docs/5.1/homestead) 的話，優雅鏈結會自動的幫你設定完成。
...
應用程式的當前環境是由 `.env` 檔案中的 `APP_ENV` 變數所決定。你可以透過 `App` [facade](/laravel_tw/docs/5.1/facades) 的 `environment` 方法取得該值：
...
當應用程式處於維護模式中，將不會處理任何[隊列工作](/laravel_tw/docs/5.1/queues)。所有的隊列工作將會在應用程式離開維護模式後繼續被進行。
