...
接著，在 `app` 設定檔中註冊 `Laravel\Cashier\CashierServiceProvider` [服務提供者](/laravel_tw/docs/5.1/providers)。
...
`subscribed` 方法很適合用在[路由中介層](/laravel_tw/docs/5.1/middleware)，讓你可以透過使用者的訂購狀態，過濾存取路由及控制器：
...
由於 Stripe webhooks 必須繞過 Laravel 的 [CSRF 驗證](/laravel_tw/docs/5.1/routing#csrf-protection)，請確定在增加 URI 例外至你的 `VerifyCsrfToken` 中介層：
...
當列出收據給客戶時，你可以使用收據的輔助方法來顯示收據的相關資訊。舉例來說，你可能希望列出每個收據至表格中，讓使用者可以簡單的下載其中一個：
    ...
    <table>
        @foreach ($invoices as $invoice)
            <tr>
                <td>{% raw %} {{ $invoice->dateString() }} {% endraw %}</td>
                <td>{% raw %} {{ $invoice->dollars() }} {% endraw %}</td>
                <td><a href="/user/invoice/{% raw %} {{ $invoice->id }} {% endraw %}">下載</a></td>
            </tr>
        @endforeach
    </table>
